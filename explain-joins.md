...
# 結合を使用するステートメントを説明する {#explain-statements-that-use-joins}
...
マージ結合は、結合の両側がソートされた順序で読み取られるときに適用される特別な種類の結合です。これは、<em>効率的なジッパーマージ</em>に似ていると説明できます。データは結合の`Build`側と`Probe`側の両方で読み取られるため、結合操作はストリーミング操作のように機能します。マージ結合は、ハッシュ結合よりもはるかに少ないメモリを必要としますが、並行して実行されません。
