...
# PD構成ファイル {#pd-configuration-file}
...
-   デフォルト値： `"pd-cluster"`
-   同じ構成のノードを持つ複数のクラスターが連続してデプロイされる場合は、異なるクラスターノードを分離するために異なるトークンを指定する必要があります。
...
-   PDを強制的に新しいクラスターとして開始し、Raftメンバーの数を`1`に変更するかどうかを決定します
-   デフォルト値： `false`
...
[TiDBダッシュボード](/dashboard/dashboard-intro.md)内蔵PDに関連する構成項目。
...
-   デフォルト値： `/dashboard`
-   リバースプロキシの背後ではなくTiDBダッシュボードにアクセスする場合は、この構成アイテムを変更し<strong>ない</strong>でください。そうしないと、アクセスの問題が発生する可能性があります。詳細については、 [リバースプロキシの背後でTiDBダッシュボードを使用する](/dashboard/dashboard-ops-reverse-proxy.md)を参照してください。
...
すべてのリージョンのレプリケーションモードに関連する構成アイテム。詳細については、 [DR自動同期モードを有効にします](/two-data-centers-in-one-city-deployment.md#enable-the-dr-auto-sync-mode)を参照してください。
