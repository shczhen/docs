...
> <strong>ノート：</strong>
>
> 楽観的モードとその制限について深く理解していない場合は、このモードを使用することはお勧めし<strong>ません</strong>。そうしないと、移行の中断やデータの不整合が発生する可能性があります。
...
## 楽観的モードの構成 {#configuration-of-the-optimistic-mode}

楽観的モードを使用するには、タスク構成ファイルの`shard-mode`項目を`optimistic`として指定します。詳細なサンプル構成ファイルについては、 [DM高度なタスク構成ファイル](/dm/task-configuration-file-full.md)を参照してください。
...
-   A / Bテストを実行する場合は、1つのシャードテーブルで<strong>のみ</strong>テストを実行してください。
