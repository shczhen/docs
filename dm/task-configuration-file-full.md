...
# DM高度なタスク構成ファイル {#dm-advanced-task-configuration-file}
...
以下は、<strong>高度な</strong>データ移行タスクを実行できるようにするタスク構成ファイルテンプレートです。
...
| `mydumpers`        | ダンプ処理ユニットの構成引数。デフォルトの構成で十分な場合は、この項目を構成する必要はありません。または、 `mydumper-thread`のみを使用して`thread`を構成できます。                                                                                                                                                                                                 |
| `loaders`          | 負荷処理ユニットの構成引数。デフォルトの構成で十分な場合は、この項目を構成する必要はありません。または、 `loader-thread`のみを使用して`pool-size`を構成できます。                                                                                                                                                                                                 |
| `syncers`          | 同期処理ユニットの構成引数。デフォルトの構成で十分な場合は、この項目を構成する必要はありません。または、 `syncer-thread`のみを使用して`worker-count`を構成できます。                                                                                                                                                                                              |
