...
-   MySQLとは異なる機能：
    ...
    -   自動増分ID：自動増分列はTiDBでグローバルに一意です。これらは単一のTiDBサーバーではインクリメンタルですが、複数のTiDBサーバー間でインクリメンタルである必要<em><strong>はなく</strong></em>、順番に割り当てられる必要もありません。
...
-   <strong>フル</strong>：このドライバーまたはORMの使用に既知の問題がないことを示します。
-   <strong>検証済み</strong>：TiDBとMySQLの互換性の違いにより、このドライバーまたはORMを使用するとエラーが発生する可能性があることを示します。

### Javaドライバー {#java-drivers}

<strong>JDBC</strong>

サポートレベル：<strong>フル</strong>
...
<strong>Maven</strong>を使用している場合は、 `<dependencies></dependencies>`に以下を追加します。
...
<strong>Gradle</strong>を使用している場合は、 `dependencies`に以下を追加します。
