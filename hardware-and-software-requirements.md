...
| :-----: | :---: | :------: | :--------: | :--------------------: | :------------: |
|   TiDB  | 16コア+ |  32 GB + |     SAS    | 10ギガビットネットワークカード（2枚推奨） |        2       |
|    PD   |  4コア+ |   8GB以上  |     SSD    | 10ギガビットネットワークカード（2枚推奨） |        3       |
|   TiKV  | 16コア+ |  32 GB + |     SSD    | 10ギガビットネットワークカード（2枚推奨） |        3       |
...
| Blackbox_exporter |   9115   | TiDBクラスターのポートを監視するために使用されるBlackbox_exporter通信ポート |
...
| TiUP         | <li>制御マシン：単一バージョンのTiDBクラスターをデプロイするために必要なスペースは1GB以下です。複数のバージョンのTiDBクラスターがデプロイされている場合、必要なスペースが増加します。</li><li>デプロイメントサーバー（TiDBコンポーネントが実行されるマシン）：TiFlashは約700 MBのスペースを占有し、その他のコンポーネント（PD、TiDB、TiKVなど）はそれぞれ約200MBのスペースを占有します。クラスターの展開プロセス中、TiUPクラスターは一時ファイルを格納するために1 MB未満の一時スペース（ `/tmp`ディレクトリー）を必要とします。</li> | 該当なし       |
| Ngmonitoring | <li>Conprof：3 x 1 GB xコンポーネントの数（各コンポーネントは1日あたり約1 GB、合計3日を占有します）+20GBの予約済みスペース</li><li>トップSQL：30 x 50 MB xコンポーネントの数（各コンポーネントは1日あたり約50 MB、合計30日を占めます）</li><li> ConprofとTopSQLは予約済みスペースを共有します</li>                                                                                                          | 該当なし       |
